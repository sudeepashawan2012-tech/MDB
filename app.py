...
if "password_correct" not in st.session_state:
    ...
    if df is not None:
        ...
        if active_report == "🕒 Ghat Delay Report":
            ...
            st.info("Criteria: Metal Issued, Diamond NOT Issued. Delay > 5 days (Small <= 5cts) or > 9 days (Big > 5cts).")
            
            ghat_df = df.copy()
            ghat_df['METAL_ISSUE_DT'] = pd.to_datetime(ghat_df[col_issue_dt], dayfirst=True, errors='coerce')
            
            # Mask: Metal Issued is not blank AND Diamond Issued is blank
            ...
            if not final_ghat.empty:
                ...
                with f4: date_range = st.date_input("Metal Issue Date Range", [final_ghat['METAL_ISSUE_DT'].min(), final_ghat['METAL_ISSUE_DT'].max()])
            ...
            if not final_delay.empty:
                ...
                with f3:
                    date_range = st.date_input("Filter by Order Date Range", [final_delay['ORDER_DATE_DT'].min(), final_delay['ORDER_DATE_DT'].max()])
                ...
                h1.markdown("**Customer**"); h2.markdown("**Order Date**"); h3.markdown("**Bag No**"); h4.markdown("**Order Type**"); h5.markdown("**Delay**"); h6.markdown("**Karigar**"); h7.markdown("**Design**")
                st.divider()

                for _, row in final_delay.iterrows():
                    c1, c2, c3, c4, c5, c6, c7 = st.columns([1.2, 1, 1.2, 1, 0.8, 1, 1.5])
                    c1.write(row[col_cust]); c2.write(clean_date(row['ORDER_DATE'])); c3.write(f"**{row[col_bag]}**"); c4.write(row[col_order_type]); c5.write(f"⚠️ {int(row['CAD_DELAY'])} Days"); c6.write(row.get('KARIGAR', '---'))
                    img_url = row.get('IMAGE_LINK')
                    if img_url and str(img_url).strip() not in ["", "---", "None"]:
                        file_id = str(img_url).split("id=")[1].split("&")[0] if "id=" in str(img_url) else (str(img_url).split("d/")[1].split("/")[0] if "d/" in str(img_url) else None)
                        if file_id:
                ...
                st.success("✅ No CAD delays found.")
            ...
            st.write("") 
            display_section("Customer Orders", df[is_cust]); display_section("Stock Orders", df[is_stock]); display_section("Metal Issued Customer Orders", df[issued_mask & is_cust]); display_section("Metal Pending Customer Orders", df[~issued_mask & is_cust]); display_section("Metal Issued Stock Orders", df[issued_mask & is_stock]); display_section("Metal Pending Stock Orders", df[~issued_mask & is_stock])
            ...
            if search_bag:
                ...
                if not match.empty:
                    r = match.iloc[0]; col_det, col_img = st.columns([2, 1])
                    with col_det:
                        st.markdown("### 📦 Bag Master Details")
                        sub1, sub2 = st.columns(2)
                        with sub1: st.write(f"**Customer:** {r.get(col_cust)}"); st.write(f"**Type:** {r.get(col_order_type)}"); st.write(f"**Karigar:** {r.get('KARIGAR')}"); st.write(f"**Metal:** {std_round(r.get(col_metal))}g 18kt"); st.write(f"**Dia:** {float(r.get(col_dia, 0)):.2f} Cts")
                        with sub2: st.write(f"**Ordered:** {clean_date(r.get('ORDER_DATE'))}"); st.write(f"**Metal Iss:** {clean_date(r.get(col_issue_dt))}"); st.write(f"**Deliv Dt:** {clean_date(r.get('DELIVERY_DATE'))}"); st.write(f"**Status:** {r.get(col_status)}")
                    ...
                    with q1: st.markdown("**🛠️ GHAT DETAILS**"); st.write(f"**QC:** {get_val_flex('GHAT_QC')}"); st.write(f"**Date:** {get_val_flex('GHAT_DATE')}")
                    with q2: st.markdown("**💎 SETTING DETAILS**"); st.write(f"**QC:** {get_val_flex('SETTING_QC')}"); st.write(f"**Date:** {get_val_flex('SETTING_DATE')}")
                    with q3: st.markdown("**✨ FINAL FINISH**"); st.write(f"**Final QC:** {get_val_flex('FINAL_QC')}"); st.write(f"**QC Date:** {get_val_flex('FINAL_QC_DATE')}")
            ...
            if sdf is not None:
                try:
                    ...
                    s_report = pd.DataFrame({'Customer': sdf.iloc[:, 0].astype(str).str.strip(), 'Dia_Cts': pd.to_numeric(sdf.iloc[:, 11], errors='coerce').fillna(0), 'Date': pd.to_datetime(sdf.iloc[:, 19], dayfirst=True, errors='coerce')})
                    s_report = s_report.dropna(subset=['Date']); s_report = s_report[s_report['Date'].dt.year == 2026]
                    if not s_report.empty:
                        s_report['Month'] = s_report['Date'].dt.strftime('%B')
                        month_order = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
                        fig_cust = px.bar(s_report.groupby(['Month', 'Customer'], observed=True)['Dia_Cts'].sum().reset_index(), x="Month", y="Dia_Cts", color="Customer", barmode="group", text_auto='.2f', category_orders={"Month": month_order}, template="plotly_dark")
